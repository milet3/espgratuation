...
static char URLdata[8][4] = {"%2B", "%20", "%2F", "%3F", "%25", "%23", "%26", "%3D"};
...
void MQTT_Init(void)
{
    ...
    snprintf(Mqtt_Password, sizeof(Mqtt_Password), "version=2018-10-31&res=%s&et=%s&method=sha1&sign=%s", token.resURL, UNIX, token.signURL);
    ...
    snprintf(TopicBuff[0], sizeof(TopicBuff[0]), "$sys/%s/%s/thing/property/set", GW_PRODUCTID, GW_DEVICENAME);      // 属性设置下发
    snprintf(TopicBuff[1], sizeof(TopicBuff[1]), "$sys/%s/%s/thing/sub/login/reply", GW_PRODUCTID, GW_DEVICENAME);   // 子设备登录应答
    snprintf(TopicBuff[2], sizeof(TopicBuff[2]), "$sys/%s/%s/thing/sub/logout/reply", GW_PRODUCTID, GW_DEVICENAME);  // 子设备登出应答
    snprintf(TopicBuff[3], sizeof(TopicBuff[3]), "$sys/%s/%s/thing/pack/post/reply", GW_PRODUCTID, GW_DEVICENAME);   // 数据打包上报应答
    snprintf(TopicBuff[4], sizeof(TopicBuff[4]), "$sys/%s/%s/thing/sub/property/set", GW_PRODUCTID, GW_DEVICENAME);  // 子设备属性设置下发
    snprintf(TopicBuff[5], sizeof(TopicBuff[5]), "$sys/%s/%s/ota/inform", GW_PRODUCTID, GW_DEVICENAME);              // OTA 升级通知
	TopicNum = 6;
    
    // 注意：在 ESP-IDF 的 MQTT 库中，不需要再手动拼接 0x10, 0x82 等底层的 MQTT 16进制报文。
    // 那些基于底层数组的拼接函数全部被 ESP-IDF 内部的 mqtt_client API 取代。
}
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_DATA:
        ...
        // 打印收到的主题和数据内容
        // 注意：event->topic 和 event->data 不是以 '\0' 结尾的字符串，必须配合 len 使用 %.*s 打印
        printf("TOPIC=%.*s\r\n", event->topic_len, event->topic);
        printf("DATA=%.*s\r\n", event->data_len, event->data);
        
        // --- 业务逻辑处理区 ---
        // 在这里，你可以根据 event->topic 来判断是哪个主题下发的数据，并调用不同的处理函数
        // 示例：
        // if (strncmp(event->topic, TopicBuff[0], event->topic_len) == 0) {
        //     // 处理属性设置下发
        //     Process_Property_Set(event->data, event->data_len);
        // } else if (strncmp(event->topic, TopicBuff[5], event->topic_len) == 0) {
        //     // 处理 OTA 通知
 ...
 */
int esp_mqtt_publish_msg(const char *topic, const char *data, int len, int qos, int retain)
{
    if (mqtt_client == NULL) {
