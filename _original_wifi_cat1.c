...
static bool looks_like_hex_string(const char *value) {
  if (value == NULL) {
    return false;
  }
...
static bool looks_like_decimal_string(const char *value) {
...
static bool looks_like_base64_secret(const char *value) {
  if (value == NULL || value[0] == '\0') {
    return false;
  }
...
static const char *json_string_or_fallback(cJSON *primary, cJSON *fallback,
...
typedef enum {
  OTA_CHECK_PARSE_INCONCLUSIVE = 0,
...
static esp_err_t ota_query_device_id_via_devinfo(const char *authorization,
                                                 char *out_dev_id,
...
static esp_err_t ota_query_device_id(const char *authorization, char *out_dev_id,
                                     size_t out_dev_id_len) {
