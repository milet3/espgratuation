...
# Find the range of functions to move
move_start = None
move_end = None
for i, line in enumerate(lines):
    if "OTA notify/reboot coordination stubs" in line:
        move_start = i
    if move_start is not None and i == len(lines) - 1:
        move_end = i

# Find where Studio_OTA_CheckTask starts
insert_at = None
for i, line in enumerate(lines):
    if "void Studio_OTA_CheckTask(void)" in line.strip() and "{" in line:
        insert_at = i
        break
...
if move_start and move_end and insert_at:
    ...
    print(f"Moved {len(moved_lines)} lines to before Studio_OTA_CheckTask (line {insert_at+1})")
else:
    print(f"move_start={move_start}, move_end={move_end}, insert_at={insert_at}")
